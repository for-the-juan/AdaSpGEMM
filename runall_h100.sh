...
AA="/data/home/scyf211/run/TileSpGEMMDataset"

mkdir -p ./log_TileSpGEMM
...
for mtx_file in "$AA"/*.mtx; do
    base_name_=$(basename $mtx_file .mtx)
    mkdir -p ./log_TileSpGEMM/${base_name_}
    for ((i=0; i<length; i++)); do
        m=${TILE_SIZE_M[i]}
        n=${TILE_SIZE_N[i]}
        ./bin/test_m${m}_n${n} -d 0 -aat 0 $mtx_file > log_TileSpGEMM/${base_name_}/aat0_m${m}_n${n}.log
